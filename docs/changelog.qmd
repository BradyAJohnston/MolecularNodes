...
- `Animate Ease`, `Animate Stagger` and `Animate Reveal` nodes under Animate: Penner easing curves (In / Out / In Out) for any 0..1 value or field, a per-point stagger factor that starts later for each residue, chain, atom or attribute value, and a node that fades (`Color` alpha), grows (`vdw_radii`) or culls atoms by a factor.
