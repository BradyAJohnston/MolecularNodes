...
## Added
- `Canvas.timeline()`, a storyboard layer that compiles camera moves (`canvas.camera.look_at`, `orbit`, `dolly`, `zoom`, `move_to`, `focus`), value tweens on node inputs, entity and annotation properties, and trajectory playback (`traj.play(a, b)`) into ordinary keyframes. The camera is now parented to a `camera_pivot` empty so an orbit is one pair of keys; `Camera.location` and `Camera.rotation` are world-space. `entity.styles` gives the style nodes of an entity's tree by index or name for animating from the simple API.
- A storyboard gallery in the docs (`docs/examples/timeline/`): nineteen scripts that reproduce ProteinMotion's examples with the timeline layer, with their renders, each noting what Molecular Nodes cannot do yet.
