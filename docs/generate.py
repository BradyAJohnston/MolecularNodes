...
LINKS_LUA_FILE = DOCS_FOLDER / "_node_links.lua"
...
def write_node_links_lua(named_links: dict[str, str]) -> None:
    """Write the node name -> GUI entry map read by docs/filters.lua, which
    resolves `Name::Node` inline code into links to the node documentation."""
    lines = [
        "-- Generated by docs/generate.py: node name -> its entry on the GUI node",
        "-- pages. Used by docs/filters.lua to resolve `Name::Node` links.",
        "return {",
    ]
    for name, href in sorted(named_links.items()):
        key = name.replace("\\", "\\\\").replace('"', '\\"')
        lines.append(f'  ["{key}"] = "{href}",')
    lines.append("}")
    LINKS_LUA_FILE.write_text("\n".join(lines) + "\n")
...
def generate_node_sections() -> dict[str, str]:
    ...
    for (category, sub), items in pages.items():
        ...
        for group, path in items:
            ...
            href = f"/nodes/{slug}.qmd#{_anchor(group.name)}"
            links[f"molecularnodes.{path}"] = href
            named_links[group.name] = href
...
def generate_custom_nodes_page() -> dict[str, str]:
    extras: dict[str, dict] = yaml.safe_load((DOCS_FOLDER / "nodes.yml").read_text())
    named_links: dict[str, str] = {}
    with open(NODES_FOLDER / "generated_nodes.qmd", "w") as file:
        ...
        for name, extra in extras.items():
            ...
            named_links[label] = f"/nodes/generated_nodes.qmd#{anchor}"
            file.write(f"\n## {label} (`{name}`) {{#{anchor}}}\n")
            if extra.get("description"):
                file.write(f"\n{extra['description']}\n")
            if extra.get("videos"):
...
if __name__ == "__main__":
    named_links = generate_node_sections()
    named_links |= generate_custom_nodes_page()
    write_node_links_lua(named_links)
    generate_data_table()
