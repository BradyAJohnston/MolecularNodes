...
Move an instanced assembly to the world origin. Some structures are not centered on the world origin, so this node can reset them to the world origin for convenient rotation and translation and animation.
...
Choose a color for an indivudual element. Select the element based on `atomic_number`. Useful for higher atomic number elements which are less commonly found in structures.
...
Choose a color for the different secondary structures, based on the `sec_struc` attribute.
