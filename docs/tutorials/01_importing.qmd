...
Rather than starting Blender with the default scene, you can start it using the _Molecular Nodes_ template by choosing `Molecular Nodes` from the splashscreen.

All of the features of Molecular Nodes are available regardless, but starting from a template will have some quality of life improvements like better default lighting, & panel layouts. These tutorials will assume you are using the template.

Opening this template is also accessible through the `File` -> `New` -> `Molecular Nodes`.

![Start using the Molecular Nodes preset.](https://imgur.com/S4whNMC.png)


## Import Panel

Molecules, simulations and other file formats can all be imported via the `Molecular Nodes` subpanel. This panel can be found under the `Scene Properties` panel. This is the small cone and spheres icon on the right. Inside of the MN panel, you can change the import method with the drop down menu. By default it will be set to download structures from the wwPDB based on the 4-character code.

Below are the different options for importing structures from the PDB. These will change depending on the method chosen for importing.

![The panel location.](https://imgur.com/J3yr5y1.png)

In this example, you can choose the starting style for the structure (this can be changed later), whether to cache the downloaded structures, whether to build the associated biological assemblies, centre the structure on the world origin, or remove the solvent from the structure such as waters.

![](https://imgur.com/dztZ1TY.mp4)
...
Rather than having to directly interact with the node tree, with the molecule selected, you can also interact with the style options via the `Object` tab in the Molecular Nodes panel. This will let you adjust settings that are part of the node tree, as well as quickly changing the used style. This is intended for quickly tweaking some aspects of basic styles, and not for interacting with more complex node trees.

![Adjusting style inputs](https://imgur.com/PnrKqfb.mp4)

In this example video we can see that the panel and node tree are adjusting exactly the same values, and chaning one changes both
