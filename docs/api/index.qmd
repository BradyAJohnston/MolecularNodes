...
Because of this, the `bpy` package upon which `molecularnodes` is build is locked to `python==3.13` and in particular `numpy>=2.3`. Going outside of these very particular versions is recipe for disaster.
...
- [](`~mn.Molecule`)
    ...
    - Structure files are parsed with [](`biotite`) and converted to a `mda.Universe`
...
To get started, with import the module start a new scene be creating out [](`~mn.Canvas`) object. This class lets us more easily control the 3D scene that we create our models and do rendering with. Changing render engine, camera resolution and controlling the camera is all done through this class.
