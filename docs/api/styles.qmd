...
- `material` param
  - One of the pre-built materials from `mn.material`, e.g.
    `mn.material.AmbientOcclusion(distance=0.5)`, a Blender material, or the name of
    a pre-built material, e.g. `Default` or `Squishy`.
- `color` param
...
```{python}
mol.add_style("surface", selection="resid 100:150", material="Flat")
canvas.look_at(mol)
canvas.snapshot()
```
...
for code, style in zip(codes, styles):
    ...
    mol.add_style(style, material="Flat")
...
mol = (
    ...
    .add_style("cartoon", material="Ambient Occlusion")
    .add_style(
        style="surface",
        selection="protein",
        material="Default",
...
mol = (
    ...
    .add_style("cartoon", selection="is_peptide", material="Ambient Occlusion")
    .add_style("ball_and_stick", selection="not protein", material="Default", bond_find=True)
)
canvas.look_at(mol)
canvas.snapshot()
```
