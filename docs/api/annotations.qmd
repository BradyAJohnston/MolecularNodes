...
- [`add_atom_info()`](`~mn.entities.molecule.annotations.AtomInfo`) - For some basic atom info
- [`add_com()`](`~mn.entities.molecule.annotations.COM) - For center-of-mass of a selection
- [`add_com_distance()`](`~mn.entities.molecule.annotations.COMDistance`) - For distance between two center-of-masses
- [`add_canonical_dihedrals()`](`~mn.entities.molecule.annotations.CanonicalDihedrals`) - For canonical dihedrals of a residue
- [`add_universe_info()`](`~mn.entities.molecule.annotations.UniverseInfo`) - For universe info
- [`add_label_2d()`](`~mn.entities.molecule.annotations.Label2D`) - For adding a generic 2d label in viewport / render
- [`add_label_3d()`](`~mn.entities.molecule.annotations.Label3D`) - For adding a generic 3d label
...
```{python}
# frame the view and render
canvas.look_at(t.get_view(), viewpoint="front")
canvas.snapshot()
```
...
Custom annotations get automatically registered with the [](`~mn.entities.molecule.annotations.MoleculeAnnotationManager`). They can be manually unregistered and re-registered using the [](`~mn.annotations.manager.BaseAnnotationManager.unregister_type`) and [](`~mn.annotations.manager.BaseAnnotationManager.register_class`) methods if required.
...
These methods can be used within the [](`~molecularnodes.annotations.base.BaseAnnotation.draw`) method of a custom annotation. Please see the API reference for [](`mn.annotations.base`) for details about these methods.
