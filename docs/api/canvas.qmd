...
All entities have a `get_view` method that returns the entity object by default. The [](`~molecularnodes.entities.Trajectory.get_view`) method of trajectories can return views based on one or more selection strings or `AtomGroups` and also take the specific trajectory frame into consideration.
