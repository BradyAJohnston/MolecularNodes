...
t = mn.Molecule(u).add_style("cartoon")
```
...
- [`add_atom_info()`](`~molecularnodes.entities.molecule.annotations.AtomInfo`) - For some basic atom info
- [`add_com()`](`~molecularnodes.entities.molecule.annotations.COM) - For center-of-mass of a selection
- [`add_com_distance()`](`~molecularnodes.entities.molecule.annotations.COMDistance`) - For distance between two center-of-masses
- [`add_canonical_dihedrals()`](`~molecularnodes.entities.molecule.annotations.CanonicalDihedrals`) - For canonical dihedrals of a residue
- [`add_universe_info()`](`~molecularnodes.entities.molecule.annotations.UniverseInfo`) - For universe info
- [`add_label_2d()`](`~molecularnodes.entities.molecule.annotations.Label2D`) - For adding a generic 2d label in viewport / render
- [`add_label_3d()`](`~molecularnodes.entities.molecule.annotations.Label3D`) - For adding a generic 3d label
...
### [atom_info](`~molecularnodes.entities.molecule.annotations.AtomInfo`)
...
```{python}
# add a style to display all the alpha carbons as ball and stick
t.add_style("ball_and_stick", selection="name CA")
...
canvas.look_at(t.get_view("resid 73:78"), viewpoint="front")
canvas.snapshot()
```
...
### [com](`molecularnodes.entities.molecule.annotations.COM`)
...
```{python}
# frame the view and render
canvas.look_at(t, viewpoint="front")
canvas.snapshot()
```
...
### [com_distance](`molecularnodes.entities.molecule.annotations.COMDistance`)
...
```{python}
# frame the view and render
canvas.look_at(t.get_view("resid 1 129"), viewpoint="bottom")
canvas.snapshot()
```
...
### [canonical_dihedrals](`molecularnodes.entities.molecule.annotations.CanonicalDihedrals`)
...
```{python}
# frame the view and render
canvas.look_at(t.get_view("resid 200"), viewpoint="back")
canvas.snapshot()
```
...
### [universe_info](`molecularnodes.entities.molecule.annotations.UniverseInfo`)
...
```{python}
# frame the view and render
canvas.look_at(t.get_view(), viewpoint="top")
...
### [simulation_box](`molecularnodes.entities.molecule.annotations.SimulationBox`)
...
### [label_2d](`molecularnodes.entities.molecule.annotations.Label2D`)
...
```{python}
# frame the view and render
canvas.look_at(t.get_view(), viewpoint="left")
canvas.snapshot()
```
...
### [label_3d](`molecularnodes.entities.molecule.annotations.Label3D`)
...
```{python}
# show the alpha carbon of resid 1 as ball and stick
t.add_style("ball_and_stick", selection="resid 1 and name CA")
...
```{python}
# frame the view and render
canvas.look_at(t.get_view("resid 1"), viewpoint="front")
canvas.snapshot()
```
...
#### The [](`~molecularnodes.annotations.base.BaseAnnotation.defaults`) method
...
#### The [](`~molecularnodes.annotations.base.BaseAnnotation.validate`) method
...
#### The [](`~molecularnodes.annotations.base.BaseAnnotation.draw`) method

This is the main draw method of the annotation that gets called multiple times as part of Blender's 3D viewport draw handler and during rendering. It is important to only do the minimum required drawing stuff in this method to avoid viewport lag. The [](`~molecularnodes.annotations.base.BaseAnnotation.defaults`) and [](`~molecularnodes.annotations.base.BaseAnnotation.validate`) methods above can be used to setup all the required analysis classes and results that can be displayed in this method.
...
```{python}
# frame the view and render
canvas.look_at(t.get_view("resid 75"), viewpoint="front")
canvas.snapshot()
```
...
Custom annotations get automatically registered with the [](`~molecularnodes.entities.molecule.annotations.MoleculeAnnotationManager`). They can be manually unregistered and re-registered using the [](`~molecularnodes.annotations.manager.BaseAnnotationManager.unregister_type`) and [](`~molecularnodes.annotations.manager.BaseAnnotationManager.register_class`) methods if required.
...
| Type | Representation in Blender GUI |
|:---|:---|
|`str`|String input|
|`bool`|Boolean checkbox|
|`int`|Integer input|
|`float`|Float input|
|`tuple[float, float]`|A vector input in 2 dimensions|
|`tuple[float, float, float]`|A vector input in 3 dimensions|
|`tuple[float, float, float, float]`|A vector input in 4 dimensions. Input names that contain the word "color" will become Color selections|
|`list[str]`|An Enumeration drowpdown|

### Param overrides

Text, lines and meshes drawn in custom annotations can be controlled by the same params as described in the [Common Annotations Params](#common-annotation-params) section above. Custom annotations can also override these parameters to either hardcode specific values or allow additional customization though separate inputs. This can be done by passing the `overrides` keyword param to any of the drawing utilities and specify a dictionary of the parameters to override along with their values.
...
- [](`~molecularnodes.annotations.base.BaseAnnotation.draw_text_2d`) - Draw text at a given 2D position (normalized co-ordinates) of Viewport.
- [](`~molecularnodes.annotations.base.BaseAnnotation.draw_text_3d`) - Draw text at a given 3D position
- [](`~molecularnodes.annotations.base.BaseAnnotation.draw_line_2d`) - Draw a line between two points (normalized co-ordinates) in 2D viewport space
- [](`~molecularnodes.annotations.base.BaseAnnotation.draw_line_3d`) - Draw a line between two points in 3D space
- [](`~molecularnodes.annotations.base.BaseAnnotation.draw_circle_2d`) - Draw a circle around a 2D point (normalized coordinate) in the viewport space
...
- [](`~molecularnodes.annotations.base.BaseAnnotation.draw_bpy_image`) - Draw an image from bpy.data.images at the given 2D position (normalized co-ordinates) of Viewport.
- [](`~molecularnodes.annotations.base.BaseAnnotation.bpy_image_to_pil_image`) - Convert Blender image to PIL image
- [](`~molecularnodes.annotations.base.BaseAnnotation.pil_image_to_bpy_image`) - Convert PIL image to Blender image

These methods can be used within the [](`~molecularnodes.annotations.base.BaseAnnotation.draw`) method of a custom annotation. Please see the API reference for [](`molecularnodes.annotations.base`) for details about these methods.
