...
workflow = [
    ...
    trans.center_in_box(
        protein, center="geometry"  # move atoms to center protein
    ),
    trans.wrap(
        water, compound="residues"  # wrap each water back into box
    ),
    trans.fit_rot_trans(
...
Import universe as a `Trajectory` and add a ribbon style to represent the protein.
```{python}
protein_center = np.mean(protein.atoms.positions, axis=0)

t = mn.Trajectory(u).add_style(style="ribbon")
t.add_style("spheres", selection=f"not protein and prop y >= {protein_center[1]}")

t.styles[0].backbone_radius = 2
t.styles[1].sphere_geometry = "instance"
t.styles[1].sphere_subdivisions = 4
