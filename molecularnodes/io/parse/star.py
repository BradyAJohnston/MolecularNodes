...
class StarFile(Ensemble):
    ...
    def create_model(self, name='StarFileObject', node_setup = True, world_scale=0.01):
        ...
        model = bl.obj.create_object(xyz * world_scale, collection=bl.coll.mn(), name=name)
        ...
        for col in df.columns:
            ...
            if np.issubdtype(col_type, np.number):
                bl.obj.add_attribute(model, col, df[col].to_numpy().reshape(-1), 'FLOAT', 'POINT')
            ...
            # If col_type is object, convert to category and add integer values
                ...
                model[col + '_categories'] = list(df[col].astype('category').cat.categories)
        
        if node_setup:
            bl.nodes.create_starting_nodes_starfile(model, n_images=self.n_images)
