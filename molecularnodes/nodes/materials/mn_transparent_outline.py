# Material "MN Transparent Outline", dumped by nodebpy.assets.dump_library.
...
from ..shader.transparent_outline import TransparentOutline


class MNTransparentOutline(CustomShaderGroup):
    _name = "Shader Nodetree"

    def _build_group(self, tree: TreeBuilder[ShaderNodeTree]) -> None:
        _material_output = TransparentOutline(
            alpha=0.7, menu="Outline", outline_color=(0.0, 0.0, 0.0, 1.0)
        ) >> s.MaterialOutput(is_active_output=True)


MATERIAL = MNTransparentOutline
MATERIAL_NAME = "MN Transparent Outline"
