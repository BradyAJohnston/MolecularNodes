...
def create_prop_AtomicNum_ChainNum_NameNum(univ, name, collection):
    ...
    try:
        prop_elem_num = list(map(lambda x: dict_elements.get(x, 0), u.atoms.elements))

    except:
        try:
            prop_elem = list(map(lambda x: mda.topology.guessers.guess_atom_element(x), univ.atoms.names))
            prop_elem_num = list(map(lambda x: dict_elements.get(x, 0), prop_elem))
        except:
            prop_elem = []
            for i in range(n - 1):
                prop_elem.append(0)
    ...
    try:
        ...
        prop_chain_num = np.array(list(map(lambda x: np.where(x == unique_chains)[0][0], prop_chain)))
    except:
        prop_chain_num = []
        for i in range(n - 1):
    ...
    try:
        ...
        prop_name_num = np.array(list(map(lambda x: np.where(x == unique_names)[0][0], prop_name)))
    except:
        prop_name = []
        for i in range(n - 1):
            prop_name.append(0)
...
create_prop_AtomicNum_ChainNum_NameNum(
    univ = u, 
