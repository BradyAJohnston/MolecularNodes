...
## 14. Pitfalls found while writing Select String (2026-09-14)

- **Multi-input link order is not preserved by `build`.** The build splits the Group Input
  node per socket and re-creates its links; a link from a group input into a multi-input
  socket (Join Strings) ends up first whatever the source order, so `"/" + item + "/"`
  became `item + "/" + "/"` in nodes.blend while the same source built in-session was
  right. Route the group input through another node first (`item.trim()`). When a packaged
  asset behaves differently from an in-session build, diff the two trees node by node,
  links included (`tests/data`-free scratch script, one process per tree since the
  in-session build reuses an appended group by name).
- **`dump` inlines a helper used by one asset** into that asset's module as a nested
  `CustomGeometryGroup` class and deletes its `_shared/` file. Only helpers used by two or
  more assets live in `_shared/`.
- **`g.Compare.<type>.equal(a, b)` wants sockets, not nodes.** Compare has one A/B pair per
  data type; a node argument resolves against the first "A" (float) socket. Pass
  `node.o.<socket>`.
- **String nodes vs sockets:** `g.String(string="x")` is a node; string methods
  (`.split`, `.contains`, ...) live on `.o.string`.
- **Chain and residue name lookups** in a tree: names are integers on the geometry
  (`chain_id` index into `obj.mn.chain_ids`, `res_name` from `assets/data.py`), so a node
  that takes typed names needs the lists as string inputs; `select_string.py` and its
  `String List Index` helper are the pattern, and `nodes/_handlers.py` fills the
  entity-dependent list on add.
