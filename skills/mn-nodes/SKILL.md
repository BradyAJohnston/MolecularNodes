...
1. **Branch and commit** with the usual `git checkout -b <name>`. Commit messages are
   written by the human and carry no AI author or `Co-authored-by` trailers (see
   `AI_POLICY.md`). If this skill needs updating because the workflow changed, include
   the edit in the same PR and call it out in the description.
2. **Use-case renders.** Write a short script outside the repo (see section 9 for
...
6. **Disclose AI use** in the PR's "AI disclosure" section, written by the human, as
   `AI_POLICY.md` requires. The point of this workflow is that the `.py` diff, the
   numeric tests, the goldens and the tree plots make the tree reviewable regardless
...
Viewer) before borrowing ideas. Work in a scratch directory outside the repo. A `.blend`
can carry auto-running text blocks, so open with `use_scripts=False` and check
`bpy.data.texts` before anything else. Ask the user before downloading third-party
files.
...
6. Record licence per source. CC0 and MIT copy freely; GPL-3.0 is compatible with MN but
   keep attribution in the module docstring. Write the survey up under `docs/dev/` so
