...
- The node reference structure in the API docs: `docs/generate.py` groups the node assets by their asset catalogs and writes one quartodoc page per category (per subcategory for `Utilities`) into the marked block of `docs/_quarto.yml`, which `quartodoc build` then renders. Each node appears on its category page under its node group name, documented from the generated class.

Extra information that can't live on the nodes themselves — long-form prose and demo videos of the nodes in use — lives in `docs/nodes.yml`, keyed by node group name. The custom quartodoc renderer (`docs/_renderer.py`) injects it into the rendered page for each node class, between the description and the Parameters section. Entries marked `custom: true` describe the node groups generated per imported structure (which have no class), documented on `docs/api/generated_nodes.qmd` by `docs/generate.py`. Relevant example videos should be updated in `docs/nodes.yml` when nodes are changed.
