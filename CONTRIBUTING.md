...
### Inline Tags: Types, Attributes and Node Links

`docs/filters.lua` post-processes every piece of inline code in the docs, turning a `Name::Type` suffix into a colored "chip" styled like the corresponding part of the node editor. Use these tags when writing prose so that types, attributes and nodes are visually consistent across the docs:

- **Socket / data types** — `` `Name::Type` `` renders as `Name` colored by socket type, e.g. `` `0.1::Float` ``, `` `Position::Vector` ``, `` `Transform::Matrix` ``. Valid types are the socket types: `Float`, `Int`, `Bool`, `Vector`, `Matrix`, `Rotation`, `Color`, `String`, `Geometry`, `Material`, `Object`, `Collection`, `Menu`, `Shader`, `Image`, `Bundle` and `Closure` (plus `Name` and `Input` for GUI labels). A few things are tagged automatically: `` `True` `` and `` `False` `` as `Bool`, and single-quoted values like `` `'Instance'` `` as `String`.
- **Attribute names** — bare inline code that exactly matches a known attribute name (`` `chain_id` ``, `` `vdw_radii` ``, …) is styled as an attribute automatically. When adding a new attribute to the add-on, add its name to the `attributes` list in `docs/filters.lua` and document it in `docs/attributes.qmd`.
- **Node links** — `` `Style Surface::Node` `` renders as a chip linking to that node's entry on the GUI node pages. The name must exactly match the node group's name in the `.blend` file (so `` `Topology DSSP::Node` ``, not `` `DSSP::Node` ``); for the per-structure generated node groups, use the label from `docs/nodes.yml`. An unknown name renders unlinked and prints a `(W) ... matches no documented node` warning during the build, so typos surface at build time. The name → link map is `docs/_node_links.lua`, generated by `docs/generate.py` — run that first if node links aren't resolving.

Every tag keyword needs a matching color in the `$custom-classes` map in `docs/style.scss`. When Blender adds a new socket type: add it to the `keywords` list in `docs/filters.lua`, to the `SOCKET_KEYWORDS` sets in `docs/generate.py` and `docs/_renderer.py` (which tag the generated socket tables automatically — hand-written tags are only needed in prose), and add a color variable in `docs/style.scss`, sampled from Blender itself via `NodeSocket.draw_color()`.
